...
public class BlackScholesJavaDouble {
    ...
    double[] fillRandom(double low, double high) {
    ...
    public void init() {
        ...
        r = 0.02f;
        sig = 0.30f;
        call = new double[size];
        put = new double[size];
    }

    static final double Y = 0.2316419f;
    static final double A1 = 0.31938153f;
    static final double A2 = -0.356563782f;
    static final double A3 = 1.781477937f;
    static final double A4 = -1.821255978f;
    static final double A5 = 1.330274429f;
    static final double PI = (double)Math.PI;

    double cdf(double inp) {
        double x = inp;
        if (inp < 0f) {
            x = -inp;
        }

        double term = 1f / (1f + (Y * x));
        double term_pow2 = term * term;
        double term_pow3 = term_pow2 * term;
        double term_pow4 = term_pow2 * term_pow2;
        double term_pow5 = term_pow2 * term_pow3;

        double part1 = (1f / (double)Math.sqrt(2f * PI)) * (double)Math.exp((-x * x) * 0.5f);

        double part2 = (A1 * term) +
                      (A2 * term_pow2) +
                      (A3 * term_pow3) +
                      (A4 * term_pow4) +
                      (A5 * term_pow5);
    ...
    public void scalar_black_scholes_kernel(int off) {
        double sig_sq_by2 = 0.5f * sig * sig;
        for (int i = off; i < size; i++ ) {
            double log_s0byx = (double)Math.log(s0[i] / x[i]);
            double sig_sqrt_t = sig * (double)Math.sqrt(t[i]);
            double exp_neg_rt = (double)Math.exp(-r * t[i]);
            double d1 = (log_s0byx + (r + sig_sq_by2) * t[i])/(sig_sqrt_t);
    ...
    static final VectorSpecies<Double> fsp = DoubleVector.SPECIES_PREFERRED;

    DoubleVector vcdf(DoubleVector vinp) {
        var vx = vinp.abs();
        var vone = DoubleVector.broadcast(fsp, 1.0f);
        var vtwo = DoubleVector.broadcast(fsp, 2.0f);
    ...
    public int vector_black_scholes_kernel() {
        int i = 0;
        var vsig = DoubleVector.broadcast(fsp, sig);
        var vsig_sq_by2 = vsig.mul(vsig).mul(0.5f);
        var vr = DoubleVector.broadcast(fsp, r);
        var vnegr = DoubleVector.broadcast(fsp, -r);
        for (; i <= x.length - fsp.length(); i += fsp.length()) {
            var vx = DoubleVector.fromArray(fsp, x, i);
            var vs0 = DoubleVector.fromArray(fsp, s0, i);
            var vt = DoubleVector.fromArray(fsp, t, i);
